...
- `platform_plans` seeded with four rows: Monthly (`month × 1`), Quarterly (`month × 3`), Semi-Annual (`month × 6`), Annual (`year × 1`) — see [DECISIONS.md ADR-029](DECISIONS.md#adr-029--platform-plan-supports-real-billing-intervals-monthly-quarterly-semi-annual-annual).
- `platform_subscriptions` as a period-based table (one row per billing cycle, snapshot columns, `previous_subscription_id`, exclusion constraint preventing overlap while allowing adjacent renewals — see [DECISIONS.md ADR-030](DECISIONS.md#adr-030--platform-plan-pricing-is-snapshotted-onto-each-subscription-period)/[ADR-031](DECISIONS.md#adr-031--renewal-creates-a-new-subscription-period-row-periods-are-never-mutatedextended-in-place)/[ADR-032](DECISIONS.md#adr-032--overlapping-subscription-periods-are-prevented-adjacent-renewal-periods-are-allowed)).
- `platform_invoices`, `platform_payments` (with reversal columns, not hard delete).
- `get_club_platform_access(club_id)` central function (`full`/`grace`/`blocked`) and the `auth.club_write_allowed()` wrapper — see [ARCHITECTURE.md](ARCHITECTURE.md#platform-access-strategy) and [DECISIONS.md ADR-033](DECISIONS.md#adr-033--platform-access-is-full--grace--blocked-derived-by-one-centralized-db-function).
...
**Frontend work:** none yet — a minimal Club Settings "Platform Subscription" read-only summary card only (status, next due date, days remaining), sourced from the restricted view. The full Platform Owner Control Center UI is Phase 3c.
**Security work:** all platform billing tables verified inaccessible to any non-`platform_owner` role, including Club Owner, via direct query; every new RPC and `get_club_platform_access()`/`auth.club_write_allowed()` follow the [RLS_SECURITY.md](RLS_SECURITY.md) `SECURITY DEFINER` checklist.
...
**Exit gate:** a seeded test club's subscription can be created, renewed (producing a second period row linked via `previous_subscription_id`), left to lapse into `grace` then `blocked` (verified purely via date manipulation in tests, no time-travel mocking of a cron job since none exists), and restored via a recorded payment — with `clubs.status` never once changing during any of this. No non-Platform-Owner role can access platform billing tables directly.
...
**Frontend work:** `/platform` navigation shell (Overview, Clubs, Subscriptions, Payments, Renewals, Reports, Alerts, Audit, Settings). Per-club detail view `/platform/clubs/:clubId` (Overview, Current Subscription, Subscription History, Platform Payment History, Usage, Access Status, Audit) with an Actions panel: Activate, Start Trial, Renew, Change Plan, Extend Grace Period, Suspend Club, Reactivate Club, Cancel Subscription, Record Payment, Reverse Payment — each wired to its Phase 3b RPC. Platform Overview dashboard: total/active/trial/grace/expired/suspended club counts, subscriptions expiring soon, revenue collected today/month/year, renewals due, new clubs this month. Platform Reports: Subscription Report (club/plan/period/status/start/end/grace end/price/paid/outstanding), Revenue Report (by month/plan/club/payment method), Renewal Report (due in 7/15/30 days, grace period, expired, renewed), Growth Report (new/activated/renewed/expired/cancelled clubs), Usage Report (branches/fields/staff/customers/players/bookings/enrollments per club). In-app Alerts for subscriptions expiring soon and overdue renewals (rule-based, no external notification service — consistent with [PROJECT_BRIEF](../README.md) Section 59).
**Security work:** every screen and report verified Platform-Owner-only; every sensitive action (Suspend, Cancel, Reverse Payment, Extend Grace Period, Change Plan) writes an `audit_logs` entry with actor/before/after/reason.
**Tests:** each report's output matches manually-verified totals against seeded multi-club test data; every Action button's RPC call is verified to require `platform_owner` and to write the correct audit entry; Club Owner attempting to reach any `/platform/*` route is redirected/rejected.
**Exit gate:** Platform Owner can view the Overview dashboard with correct aggregate counts, drill into a specific club's full history, perform every listed Action successfully with a corresponding audit entry, and view all five report types against seeded data with figures that match manual verification.
