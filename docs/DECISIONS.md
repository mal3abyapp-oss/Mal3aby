...
> **2026-09-04 (Sales Intelligence, Phase 14 closure) — ADR-054's tenant-conversion TRUE STOP resolved.** The identity/ownership decision left open below (two candidate resolutions, neither chosen) is now decided, by explicit user mandate: **invite-based owner activation**. `sales_win_lead_and_invite_owner()` moves a lead WON → AWAITING_OWNER_ACTIVATION in one transaction and mints a `sales_tenant_activation_invites` row (token+secret two-factor, sha256 at rest, 5-attempt shared lockout, single pending/consumed per lead — directly modeled on the proven `portal_invites` mechanism, including its two hardening fixes: column-level grants excluding hash columns, and freshness-binding on any service-role identity claim). The prospect verifies email+secret at `/sales-activate/:token` (`ActivateTenantOwnerPage.tsx`), then either creates a new pre-confirmed identity (`sales-activate-tenant-owner` Edge Function, service-role `auth.admin.createUser`, zero outbound email) or signs into an existing Mal3aby account — never an automatic email-string link. Either way, `claim_sales_activation_invite()` (authenticated-only, reads `auth.uid()` from the prospect's own real session) is the **sole** function that ever calls `complete_new_club_onboarding()` — unmodified, reused exactly as every self-service signup uses it, so a sales-converted tenant gets the identical trial/module defaults. `sales_leads.status` gained `awaiting_owner_activation`/`tenant_activated`; `sales_leads_conversion_consistency` now ties `converted_club_id`/`converted_at` to `tenant_activated` specifically (not `won`), so "status=WON alone must NOT create a tenant" is a DB-level invariant, not just RPC discipline. Idempotent throughout: a double-click/parallel-tab race re-resolves to the same `converted_club_id` rather than creating a second club, enforced by a `for update` lock on `sales_leads` plus the invite's own consumed-with-matching-identity short-circuit. Full audit trail via `sales_lead_activities` (won, invite created/resent/consumed, activation_failed, tenant_created, owner_linked, conversion_completed). See the RPC/schema/Edge-Function detail inline below where the original TRUE STOP is recorded, and `supabase/migrations/20260904120000_sales_tenant_activation_invites_schema.sql` / `20260904120100_sales_tenant_activation_invites_rpcs.sql` / `20260904120200_sales_lead_full_profile_activation_invite.sql` for the exact implementation.
