...
### Sales Intelligence structural regression (ADR-054, 2026-09-04)

`supabase/tests/sales_intelligence_structural_regression.sql` follows the identical pattern and has the identical CI status as `structural_security_regression.sql` above: 10 pure schema-shape checks (RLS enable+force coverage on all 22 `sales_*` tables, zero anon/public grants, all 12 `platform.sales.*` permission keys present and correctly seeded to `platform_owner`/`platform_admin`/`platform_support`, the duplicate-conversion-prevention constraints, zero unpinned `search_path`s, zero stale function overloads, the quota row-lock, and the hard WhatsApp-channel refusal in the outreach queue function), all verified passing live against the real project via the Supabase MCP `execute_sql` tool during this pass, none of it yet a standing CI job, blocked by the exact same upstream `supabase db reset` issue documented above — this module adds no new CI blocker, it inherits the existing one. One real regression was caught and fixed by this file during its own first live run: CHECK 4 found `platform_owner` had zero explicit `platform_role_permissions` rows for the new `platform.sales.*` keys (the historical cross-join seed from `20260826121055` does not retroactively apply to permissions added by later migrations) — functionally safe throughout (`is_platform_owner()` unconditionally bypasses the row-based check), but the missing rows broke the Role Editor UI's own display completeness; fixed by adding the explicit cross-join in the same migration.
