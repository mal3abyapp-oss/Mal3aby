...
- **A club's join/request date must be visible on the same list screen the Platform Owner already triages from, not only usable as a hidden filter input.** `PlatformClubsPage`'s `created_at` was already fetched (it powered the `createdFilter === 'this_month'` filter) but never rendered as a column — `5eff674` added the "Joined" column directly, matching the same field already driving the filter. Prefer surfacing an already-fetched field over adding a new query when the gap is display-only.
...
- **An operationally material per-staff signal that already exists on Employee 360 must also surface on the staff LIST, not only after opening each person's page, if the Club Owner needs it to triage across the whole team at a glance.** `StaffPage`'s outstanding-liability column (`5eff674`): sourced from the same real `employee_cash_liabilities` table already used by Employee 360, batched in one query for the whole list (the same convention as this file's own `profiles` batching), never a per-row RPC call. Shows the amount only when it's nonzero (`tone="danger"`), `—` otherwise — never a `0` that could be mistaken for "confirmed clean."
...
- **A machine action or entity code must never leak into customer- or staff-facing UI, in any screen, at any time.** `src/lib/domain/audit.ts`'s `actionLabel()`/`entityLabel()` fall back to the raw string by design (never throws) — that fallback is a crash-safety net for an unmapped code, not a license to ship one unmapped. Closed a 41-of-69 action / 12-of-30 entity_type coverage gap (`791a8f8`); add a new action's label in the same commit that adds the `write_audit_log()` call — coverage must reach 100% before a new action code is allowed to render anywhere a customer or staff member (not just a platform-internal admin) can see it.
...
- A new discoverability link/CTA added to a public-facing screen must be verified at a real mobile viewport (375px), not assumed responsive — confirmed live for the homepage's new customer-link (`5f544ab`), and re-confirmed live (375px, English LTR, bottom nav intact) for the portal shell during `5eff674`'s targeted regression pass.
- A new table column added to an existing `DataTable`-based screen (Platform/Staff consoles) inherits that component's existing responsive behavior — verified by reusing the exact same column pattern already proven responsive on that table (`5eff674`'s `createdAt`/`club`/`liability` columns follow the same `render`/`header` shape as every pre-existing column on those same tables), not by inventing a new layout primitive that would need its own mobile check.
...
- **A revenue/financial report row must drill down to its owning entity through a real foreign key, never a new relation invented to make the UI convenient.** `PlatformReportsPage`'s revenue tab (`5eff674`): `platform_payments` has no `club_id` column, but does have `platform_invoice_id`, and `platform_invoices` genuinely has `club_id` — the fix was the embedded-resource join (`platform_invoices(club_id, clubs(name_ar))`) through that real FK, not a schema change or a denormalized column.
...
- **No new architecture, no new top-level pages, no redesign, no new enum values, no tenant-isolation changes, no accounting-rule changes** — every fix across all six commits in this audit (`791a8f8`, `7dff2e0`, `49f07a6`, `bd1d731`, `5f544ab`, `5eff674`) was additive to an existing screen, a narrow grant-reverified RPC widening, a real-FK join, or a plain informational link — matching the audit's own explicit "don't change architecture if a natural improvement solves it" constraint throughout.
- **The second FINAL PRODUCT COMPLETENESS ROUND (`5eff674`)** closed exactly the three gaps the first round's own closure report proved still open — `PlatformClubsPage`'s missing join-date column, `PlatformReportsPage`'s missing per-club revenue drill-down, and `StaffPage`'s missing list-level cash-liability visibility — and deliberately did not re-open Invoice due-date (B2, still `REQUIRES OWNER BUSINESS DECISION`, unchanged) or a formal club-approval workflow (still not built, unchanged); both remain exactly as recorded above. Customer portal discoverability (C1) was re-investigated and confirmed already complete from the first round — the only public-booking entry point (`PublicClubBookingPage.tsx`) that could plausibly need a portal link is the post-booking confirmation screen, which already has one; adding a link to the pre-booking browsing view would have violated the standing "don't distort the public booking experience" constraint, so no code changed there this round.
