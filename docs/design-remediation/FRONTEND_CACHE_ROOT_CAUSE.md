...
**Final rule (created live 2026-09-06, second independent review, exact configuration):**
- **When incoming requests match:** `not starts_with(http.request.uri.path, "/assets/")` — i.e., everything except the hashed assets directory. (Corrected 2026-09-06 by independent review: the Rules Language calls `starts_with()` as a function with the field as its first argument — `starts_with(field, substring)` — negated with the `not` operator, never as dot-notation method syntax or `eq false`. Verified against Cloudflare's own Rules Language functions reference.)
- **Then:** Cache eligibility → **Bypass cache**.
- **Zone:** `mal3aby.app` only.
- **`workbox-<hash>.js` exception considered and deliberately NOT added**: a second independent reviewer confirmed a regex-based exception (`matches()`) would require a Business/Enterprise Cloudflare plan — this zone is confirmed on the **Free** plan, so `matches()` is unavailable and would fail to save. Leaving `workbox-*.js` bypassed at the *edge* has negligible cost: the browser's own cache is entirely unaffected by this rule (governed solely by the Worker's existing `Cache-Control: public, max-age=31536000, immutable` header on that path), so the only effect is one small JS file being re-fetched from Workers Static Assets (not a real backend with per-request cost) on an edge cache miss. Simplicity and a known-valid expression were preferred over a regex variant that cannot be created on this plan.
- Path-only matching (`http.request.uri.path` excludes the query string, confirmed via docs) — cache-busting query strings do not affect this rule's matching in either direction, consistent with the whole point of the fix.
