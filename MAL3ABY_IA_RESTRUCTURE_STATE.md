...
completed_so_far:
  ...
  - Multi-enrollment fixture verification: no real fixture existed (0 players with >1 active enrollment). Created one via direct insert (same real player, 2nd active enrollment into a different group in the same club -- schema has no unique/exclusion constraint on player_id, a genuinely supported scenario) after confirming the in-app enrollment wizard only offers unenrolled players (a real product-scoping decision, not fought). Live-verified across 3 screens: Academy's Enrollments/Subscriptions tab correctly shows both as separate active rows; Reports' academy report correctly counts 21 active enrollments (was 20) and both groups' fill counts; Players tab's search correctly shows one player row (distinct-player count unaffected, as expected). BUT opening the player's own detail dialog (PlayerStatusPanel) surfaced a REAL bug: `.maybeSingle()` on the active-enrollments query silently failed on >1 row, falsely showing "not enrolled in any group" despite 2 real active enrollments. Fixed to fetch all active enrollments and render one card per enrollment. Live-verified the fix against the same fixture — commit 2cc2e09.
  - Multi-club guardian fixture: INVESTIGATED, could NOT be re-verified live this session -- honest finding, not glossed over. Checked live: the "أحمد والد اللاعب" customer record referenced in a prior session's summary exists as exactly ONE row (club `57ce89e4-...`), with `user_id = null` (not linked to any portal auth account). `PortalProfilePage.tsx` correctly discovers multi-club records by RLS (`customers_self_service_select`: `user_id = auth.uid()`) -- confirmed via a live query that ZERO customers in this DB currently have `user_id` set AND span 2+ `club_id`s, so this scenario is genuinely unverifiable through the real portal login flow right now, not just untested. Did not fabricate a portal auth account/credentials to force a test (against safety rules on credential handling). PortalProfilePage's own code comment (lines 19-24) already documents this exact scenario was fixed in a prior session with correct reasoning, and the club-selector UI logic was re-read and looks correct, but genuine live re-verification needs either a real second `user_id`-linked club record for an existing portal-login-capable customer, or the original prior-session fixture's auth link restored -- neither exists right now. Logged as a real, standing gap for the final regression, not silently marked done.
