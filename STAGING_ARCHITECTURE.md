...
**Fix authored, applied, and live-verified (see "What is NOT done" below — corrected from an earlier draft of this document):** a small, reusable, `platform_owner`-gated RPC, `extend_club_qa_subscription(p_club_id, p_days, p_reason)`, un-suspends a club and extends its subscription window (converting a `trial`-kind row to `complimentary`, since keeping a QA fixture alive is not a real paid conversion) — with a real `write_audit_log` entry, not a raw unaudited `UPDATE`. This is genuinely reusable maintenance infrastructure, not a one-off script — a future session (or a scheduled job) can call it again whenever a QA fixture's trial next lapses, without hand-authoring another migration.

The subagent-authored draft of this migration (`20260828100000_qa_fixture_club_platform_access_maintenance.sql`, referenced in an earlier version of this document) had a real bug: its body referenced the bare column name `club_id` inside a function whose own `returns table (club_id uuid, ...)` output column shadows it — a genuine "column reference is ambiguous" error, the same class already hit and fixed earlier in this engagement in `get_gateway_transaction_status`. Per `AGENT_ORCHESTRATION_GOVERNANCE.md`, the subagent correctly stopped rather than force it through. The orchestrator (not the subagent) diagnosed and fixed this directly: `supabase/migrations/20260827234846_fix_extend_club_qa_subscription_ambiguous_column_v2.sql` (every column reference table-qualified), applied via `apply_migration`, then the one-time fixture repair applied separately via `execute_sql` using the established `set_config('request.jwt.claims', ...)` + `set local role authenticated` impersonation pattern as the real platform-owner account.
...
## What was blocked, then resolved by the orchestrator (not a remaining gap)

The subagent building this phase correctly stopped when its own `apply_migration` call for the draft fixture-repair migration failed (a real bug in its SQL, not a permission denial — see above) rather than retry around it. Per `AGENT_ORCHESTRATION_GOVERNANCE.md`, that is the correct behavior: return control to the orchestrator instead of attempting an alternate route.

**Practical consequence, resolved**: the orchestrator diagnosed the actual bug, authored the corrected migration, applied it, and separately applied the one-time fixture repair — all live-reverified. As of 2026-08-28: `clubs.status = 'active'`, `platform_subscriptions.lifecycle_status = 'active'`, `subscription_kind = 'complimentary'`, `end_at = '2027-08-27 23:49:00+00'`, `grace_period_days_snapshot = 7`. "QA Full Test Club" is healthy and usable as the E2E fixture. This was independently re-confirmed via a direct live SQL query during the Global Regression pass (2026-08-28), not merely carried forward from this document's own earlier claim.
