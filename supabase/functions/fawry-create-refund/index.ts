...
Deno.serve(async (req) => {
  ...
  if (txn.gateway !== 'fawry') {
    return jsonResponse(req, { error: 'this payment was not processed through fawry' }, 400)
  }
  ...
  return jsonResponse(req, { refund_id: refundId, provider_refund_ref: providerRef, status: 'succeeded' })
})
