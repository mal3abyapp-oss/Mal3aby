-- Phase B (government receipt integration): record_payment() has a
-- hard server-side guard requiring a valid, amount-matching official
-- collection receipt whenever the effective government policy for a
-- club/branch/field requires one for the given payment method -- every
-- caller of record_payment() (BillingPage's split-payment form,
-- approve_payment_proof, verify_manual_payment_claim) inherits this
-- guard for free.
--
-- _create_booking_internal() -- the "pay now" path used by
-- QuickBookingSheet.tsx and the staff "New Booking" flow -- has its OWN
-- separate, parallel insert into public.payments and was never touched
-- by any of the 13 government-compliance migrations (confirmed: it was
-- last redefined at 20260819180000_public_booking_policy_enforcement,
-- which predates 20260819200000, the first compliance migration). This
-- meant a government-affiliated club's staff could record a cash
-- payment and auto-confirm a booking via "pay now" with ZERO receipt,
-- even calling create_booking() directly -- a genuine server-side
-- enforcement gap, not just a missing UI field.
--
-- Fix: add the identical guard record_payment() already uses (same
-- get_effective_government_policy() resolution, same required-receipt/
-- amount-match/not-already-linked checks) to this function's own
-- payment block, and thread a new optional p_official_receipt_id
-- parameter through create_booking() so the client can supply one.
-- create_recurring_booking() always calls with p_record_payment=false,
-- so it never reaches this guard and needs no change.
...
begin
  ...
  -- Phase B: mirror record_payment()'s own government-receipt guard
  -- BEFORE any booking/invoice/payment row is written, so a rejected
  -- payment never leaves a half-created booking behind. Resolution is
  -- field -> branch -> club, same precedence as record_payment().
  if p_record_payment and p_payment_amount is not null and p_payment_amount > 0 then
    ...
    then
      ...
      where id = p_official_receipt_id and club_id = v_club_id and status = 'active';
      ...
      if v_receipt.receipt_amount != p_payment_amount then
        raise exception 'official collection receipt amount (%) does not match the payment amount (%)', v_receipt.receipt_amount, p_payment_amount;
      end if;
    end if;
  end if;
  ...
  if p_record_payment and p_payment_amount is not null and p_payment_amount > 0 then
    ...
    if v_receipt_validated then
      ...
      set payment_id = v_payment_id, invoice_id = v_invoice_id,
          customer_id = p_customer_id, booking_id = v_booking_id, updated_at = now()
      where id = p_official_receipt_id;

      perform public.write_audit_log(
        v_club_id, 'official_collection_receipt.created', 'official_collection_receipt', p_official_receipt_id,
        null,
        jsonb_build_object('payment_id', v_payment_id, 'receipt_serial', v_receipt.receipt_serial, 'amount', p_payment_amount),
        null
      );
    end if;
    ...
    perform public.write_audit_log(
      ...
      jsonb_build_object('amount', p_payment_amount, 'method', coalesce(p_payment_method, 'cash'), 'invoice_id', v_invoice_id, 'official_receipt_id', p_official_receipt_id),
      null
    );
...
-- Public wrapper: thread the new optional parameter through.
