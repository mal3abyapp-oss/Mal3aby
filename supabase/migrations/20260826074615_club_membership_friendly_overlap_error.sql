-- Add a clean pre-check for date-range overlap in sell_club_membership
-- and renew_club_membership, so a genuine overlap attempt surfaces a
-- clear business-rule message instead of a raw Postgres exclusion-
-- constraint error (23P01) reaching the client. The GiST constraint
-- itself remains as the authoritative, race-proof guarantee -- this
-- pre-check is purely a UX improvement for the common case, verified
-- live during the freeze/renewal test matrix.
