-- EMAIL DELIVERY CHANNEL -- remaining call sites (2026-08-24).
--
-- create_public_booking(): the self-service public booking flow.
-- Reuses the same 'booking-created' template as the staff-created
-- path -- queue_email_notification safely no-ops for these bookings
-- today (the public form does not currently collect a customer email
-- at all, only name/mobile/phone_e164), but wiring it now means no
-- further DB change is needed if/when the public form is extended to
-- collect an email later.
--
-- expire_stale_booking_holds(): the cron job that auto-cancels
-- payment-hold-expired bookings. Reuses the same 'booking-cancelled'
-- template as cancel_booking() -- same event type, same dedup key
-- shape, same cancel_pending_whatsapp_for_booking() cleanup call
-- (already made channel-agnostic in
-- 20260824030000_wire_email_channel_into_cancellation.sql, so no
-- further change needed there).
--
-- create_refund() (payment-refunded) is deliberately NOT wired here
-- -- the directive's explicit template list (sections 18-22) covers
-- booking-created/confirmed-paid, booking-rescheduled,
-- booking-cancelled, payment-received, and academy-payment-received
-- only. No payment-refunded email template exists yet; queuing a row
-- for a template_key the Worker cannot render would fail at send
-- time, not queue time. If refund email is required later, add the
-- template first, then wire this call site the same way.
