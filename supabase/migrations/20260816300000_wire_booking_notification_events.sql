-- Gate 7 continued: wire the first real emit_notification_event() call
-- into _create_booking_internal(), proving the domain-event pattern end
-- to end against a real business transaction rather than leaving the
-- notification core entirely unintegrated. Emits 'booking.created'
-- always, and 'booking.confirmed' additionally when the booking is
-- paid immediately (status becomes 'confirmed') -- matching Doc 3's
-- own example event list ("booking-confirmed -> send confirmation").
-- Deliberately does NOT call enqueue_notification() here -- emitting
-- the event and deciding what to actually send are separate concerns
-- (Gate 8's automation-rules layer is what maps events to enqueued
-- messages; this migration only proves the event-emission half).
