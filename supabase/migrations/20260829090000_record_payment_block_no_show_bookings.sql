-- ZERO-TRUST ANTI-FRAUD HARDENING -- Phase 9 (2026-08-29)
--
-- CF-2 / SP-001 from MAL3ABY_SECOND_PASS_AUDIT.md: cancelled/no-show
-- bookings can retain a payable invoice; record_payment() was expected
-- to block this class of write. Re-verified this pass: a fix DOES
-- exist for 'cancelled' bookings specifically (`if v_booking_status =
-- 'cancelled' then raise exception ...`), but NOT for 'no_show' -- a
-- real, distinct booking status confirmed live-present in the schema
-- (`select distinct status from bookings` returns cancelled,
-- checked_in, completed, confirmed, no_show, pending_payment).
--
-- LIVE-CONFIRMED still exploitable: a direct query found 30 real
-- production bookings with status in ('cancelled','no_show') whose
-- linked invoice is still 'issued' (payable), 3 of which are
-- specifically 'no_show' -- meaning record_payment() would still
-- accept a payment against those 3 today. This is the same underlying
-- financial-integrity gap SP-001 named, only partially closed.
--
-- Fix: widen the existing guard from a single-status check to the same
-- two-status set create_public_booking()/other booking-lifecycle logic
-- already treats as terminal-and-non-payable elsewhere in this
-- codebase. No new architecture, no new column, no new RPC -- one
-- condition widened in the one function this gap lives in.
--
-- Historical data note: the 30 existing affected rows are NOT touched
-- by this migration (directive Section 55: never mutate valuable real
-- financial/business history to make a fix "complete"). This closes
-- the write-path going forward; correcting the 30 historical rows (if
-- desired) is a separate, explicit data-remediation decision for the
-- product owner, not a security-fix action.
--
...
begin
  ...
  -- 'no_show' -- a booking the customer never attended must not remain
  -- payable, same reasoning as an explicitly cancelled one.
