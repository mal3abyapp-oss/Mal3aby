-- CUSTOMER ACTIVATION TAKEOVER GAP -- SECURITY CLOSURE follow-up fix.
--
-- _mint_portal_invite_internal's return type changed from bare `text` to
-- `TABLE(raw_token text, raw_secret text)` in
-- 20260823080000_activation_independent_secret_factor.sql, but this
-- live production function (_create_booking_internal, wired in the
-- PRIOR pass's 20260823060000_booking_activation_invite_wiring.sql)
-- still assigned the call directly to a scalar `v_activation_token
-- text` variable -- `v_activation_token :=
-- public._mint_portal_invite_internal(...)` -- which is invalid
-- PL/pgSQL for a table-returning function and would fail at runtime
-- for every real staff-created booking for a customer with no
-- user_id yet (i.e. exactly the activation-invite code path this
-- whole feature exists for).
--
-- Fix: capture both output columns via `select ... into`, add a
-- second `v_activation_secret text` variable, and thread
-- `activation_secret` into both queue_whatsapp_notification jsonb
-- payloads (booking-created unpaid path, and booking-confirmed-paid
-- path) alongside the existing `activation_token` -- required so the
-- WhatsApp template can render directive section 10's "ONE message:
-- booking summary + activation link + activation code" contract.
