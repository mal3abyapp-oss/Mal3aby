-- PRINTING PRODUCTION ACCEPTANCE (2026-08-30), Section 6/22.
--
-- sell_club_membership() and renew_club_membership() both insert the
-- invoice_items.description as a bare plan name (v_plan.name_ar) with
-- no validity/expiry date range, despite already correctly computing
-- v_end_date (and, for renewals, v_effective_start) immediately above
-- the insert. A printed membership invoice/receipt could not tell a
-- staff member or customer what period was actually purchased --
-- exactly the same defect class as D6 (academy enrollment description),
-- fixed here the same way: append a human-readable "from -> to" date
-- range to the line description. No RETURNS TABLE shape change in
-- either function, so a plain CREATE OR REPLACE is sufficient -- no
-- DROP FUNCTION / re-GRANT required.
...
begin
  ...
  v_description := v_plan.name_ar || ' — ' || to_char(p_start_date, 'YYYY-MM-DD') || ' → ' || to_char(v_end_date, 'YYYY-MM-DD');
...
begin
  ...
  v_description := v_plan.name_ar || ' — ' || to_char(v_effective_start, 'YYYY-MM-DD') || ' → ' || to_char(v_end_date, 'YYYY-MM-DD');
