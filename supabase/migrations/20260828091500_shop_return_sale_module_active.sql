...
-- the four prior fixes this session. create_shop_sale already checks
-- _shop_module_active(club_id); return_shop_sale (the write RPC for
-- processing a return/refund against an existing sale) did not. This
-- is a real gap on a financially-sensitive write path: a Platform
-- Owner disabling Shop for a club would not stop staff from still
-- processing returns/refunds against that club's historical sales.
-- Fixed the same way as every prior fix this session: the module-
-- active check is inserted immediately after the existing
-- authorization OR-check, applying to the whole caller including a
-- platform-support session, matching every other write RPC in this
-- module.
create or replace function public.return_shop_sale(p_sale_id uuid, p_lines jsonb, p_restock boolean, p_refund_amount numeric default null, p_reason text default null, p_idempotency_key uuid default null)
returns uuid
language plpgsql
security definer
