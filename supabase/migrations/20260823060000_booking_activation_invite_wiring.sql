-- CUSTOMER ACCOUNT / CLUB PORTAL -- wire the zero-cost activation
-- invite into the staff booking-creation WhatsApp message (amendment
-- sections 3-4, 47-49): "ONE combined message" containing the normal
-- booking confirmation PLUS an activation CTA, only when the canonical
-- customer has no linked portal account yet. Never a second/separate
-- message. An already-activated customer's booking message is
-- completely unchanged from before this migration.
--
-- _create_booking_internal is redefined here byte-identical to its
-- live definition EXCEPT: (a) one new lookup of the customer's
-- user_id right where customer_name is already fetched, (b) one new
-- v_activation_token computed via _mint_portal_invite_internal only
-- when user_id IS NULL, passed to _create_portal_invite_internal with
-- this booking's id as the triggering booking (so the activation
-- page can show the minimal booking summary per amendment section 7),
-- and (c) 'activation_token' added to both existing
-- queue_whatsapp_notification jsonb payloads (unpaid + paid branches)
-- -- null when the customer is already activated, so templates.ts's
-- renderer can render the CTA conditionally with zero schema
-- assumption about what "activated" means.
...
begin
  ...
  -- CUSTOMER ACCOUNT / CLUB PORTAL -- amendment sections 3-4: only
  -- mint an activation invite (and only include its CTA in the
  -- upcoming WhatsApp message) when this customer genuinely has no
  -- linked portal account yet. An already-activated customer's
  -- message stays exactly as it was before this migration.
  if v_customer_user_id is null then
    v_activation_token := public._mint_portal_invite_internal(
