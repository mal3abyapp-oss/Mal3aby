-- CLUB MEMBERSHIPS domain -- Customer Portal self-service purchase/
-- renewal RPCs.
--
-- Design decision (post-discovery, made live during backend testing):
-- sell_club_membership/renew_club_membership are gated on STAFF
-- permissions (club_membership.create/.renew) -- a genuine authenticated
-- portal customer legitimately holds NEITHER, by design (customers are
-- not staff). The directive's Section 96 "Customer Purchase Flow
-- (Portal)... reusing existing authenticated customer identity" requires
-- a distinct self-service entry point.
--
-- Academy's own portal (get_my_portal_academy) is confirmed READ-ONLY --
-- no self-service purchase/renewal RPC exists anywhere in this codebase
-- for Academy subscriptions; the only genuine "customer purchases
-- without staff" precedent is create_public_booking, which is a fully
-- anonymous, club-public-link flow with its own from-scratch customer
-- creation logic (no auth.uid() context at all) -- NOT a fit for an
-- already-authenticated portal customer.
--
-- These two RPCs are the correct, minimal, first-class analog: same
-- business rules as the staff RPCs (server-computed dates, snapshot
-- integrity, overlap protection, branch-scope enforcement, idempotency),
-- but authorization is `customers.user_id = auth.uid()` (hard-coded
-- inside the RPC body, mirroring get_my_portal_club_memberships' own
-- pattern) instead of a staff permission check. Only allow_renewal is
-- additionally enforced here (a customer should never be able to
-- self-renew a plan the club marked non-renewable, even though staff
-- technically also check this same flag).
...
begin
  ...
  -- Self-service renewal always uses the SAME plan (no plan-change UI
  -- for the customer path -- plan change on renewal remains a
  -- staff-assisted action via renew_club_membership).
  select * into v_plan from public.club_membership_plans
...
grant execute on function public.purchase_club_membership_self_service(uuid, uuid, uuid, date, uuid) to service_role, authenticated;
grant execute on function public.renew_club_membership_self_service(uuid, uuid) to service_role, authenticated;
revoke all on function public.purchase_club_membership_self_service(uuid, uuid, uuid, date, uuid) from public, anon;
revoke all on function public.renew_club_membership_self_service(uuid, uuid) from public, anon;
