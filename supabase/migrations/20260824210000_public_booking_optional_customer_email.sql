-- FIX (confirmed gap, medium severity): the public booking form has no
-- email field at all -- not even optional -- so create_public_booking()
-- has never had a way to receive one. Product decision: email stays
-- OPTIONAL on public booking (not required); this migration only adds
-- the plumbing so the RPC can accept and persist it when the customer
-- chooses to provide it.
--
-- What changes vs. the current function
-- (20260824130000_public_booking_restore_quarantine_exclusion.sql):
--   1. New trailing parameter p_customer_email text DEFAULT NULL --
--      appended AFTER p_source so no existing caller (there is exactly
--      one: PublicClubBookingPage.tsx) breaks by omitting it.
--   2. The email is normalized the same way upsert_customer() already
--      does it: nullif(trim(p_customer_email), '') -- empty string and
--      whitespace-only collapse to NULL, never stored as ''.
--   3. On a brand-new customer INSERT, the normalized email is stored
--      on public.customers.email.
--   4. On an existing-customer match (exact-name path), the email is
--      backfilled only when the customer doesn't already have one on
--      file (coalesce(customers.email, v_email)) -- a returning
--      customer who once gave staff their email is never overwritten
--      by a public-booking submission that left the field blank or
--      typed a different address; this mirrors the "don't clobber an
--      existing identity" caution already used for full_name/consent
--      elsewhere in this function, and is deliberately NOT the same
--      "explicit clear" semantics as upsert_customer's staff-facing
--      form (there is no staff operator confirming an intentional
--      clear here).
--   5. The email is passed into the existing queue_email_notification
--      call's event payload as 'customer_email' so the email worker has
--      it available without a second lookup.
--
-- Nothing else changes: validation, pricing, dedup/quarantine lookup,
-- name-mismatch/consent-reconfirmation logic, booking/invoice/QR/
-- WhatsApp-notification logic are copied verbatim from the prior
-- version. No signature reordering, no grant change, no destructive DDL.
...
begin
  ...
  -- Same normalization upsert_customer() already applies: blank/
  -- whitespace-only collapses to NULL, never stored as ''.
  ...
  if v_customer_id is null then
    ...
    -- UPDATE path already does for staff-created customers. Email is
    -- only ever backfilled here (coalesce), never overwritten -- an
    -- address a customer already gave (e.g. to staff) is not silently
    -- replaced or cleared by a public-booking submission that left the
    -- field blank.
