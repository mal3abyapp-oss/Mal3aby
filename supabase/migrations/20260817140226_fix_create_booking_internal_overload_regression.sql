-- Fix for a real bug introduced by the immediately-preceding migration
-- (20260817130000_whatsapp_template_data_enrichment.sql): that
-- migration's _create_booking_internal() declared its parameters as
-- (..., p_record_payment, p_payment_amount, p_payment_method, ...) but
-- the function's ACTUAL live signature (the one create_booking() --
-- the real public entry point every booking flow calls -- has always
-- positionally called) is (..., p_record_payment, p_payment_method,
-- p_payment_amount, ...). Postgres treats different parameter ORDER as
-- a genuinely different function identity even when the names match,
-- so `create or replace function` did not replace the live function
-- at all -- it silently created a second, never-called overload.
--
-- Confirmed live via pg_proc: two overloads existed
-- (has_enrichment=false on the one create_booking() actually calls,
-- has_enrichment=true on the orphaned new one). This meant: (1) the
-- whole point of the enrichment migration -- club_name/timezone/sport/
-- customer_name/booking_ref reaching WhatsApp templates -- was NOT
-- actually live for real bookings (confirmed live: a real booking made
-- after that migration still queued the old narrow variables shape),
-- and (2) worse, the orphaned new overload I wrote was missing the
-- `club_write_allowed(v_club_id, 'new_commitment')` commercial
-- entitlement check present in the real live function -- since it was
-- never called, this was inert, not a live security gap, but would
-- have BECOME one the moment anything called it by the "new" parameter
-- order.
--
-- Fix: recreate _create_booking_internal with the CORRECT (original,
-- actually-called) parameter order, keeping the entitlement check AND
-- the enrichment variables. Then drop the orphaned wrong-order overload
-- outright.
...
-- Drop the orphaned wrong-order overload the previous migration
-- accidentally created (never called by create_booking(), so this is
-- pure dead-code removal, not a behavior change for any real flow).
