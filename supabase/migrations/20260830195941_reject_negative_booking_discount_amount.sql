-- BOOKINGS/FIELDS PRODUCTION ACCEPTANCE, D5: `_create_booking_internal`
-- only validates `p_discount_amount` inside `if p_discount_amount > 0
-- then ... end if` (the 30%-ceiling / exceeds-total / permission
-- checks). A NEGATIVE discount amount skips that entire block
-- entirely, so it bypasses every one of those checks -- including the
-- `booking.discount.apply`/`booking.discount.override` permission
-- gates -- and is stored/used as-is.
--
-- Live-reproduced this session: create_booking with
-- p_discount_amount = -50 on a 120 EGP booking succeeded with ZERO
-- error. The stored invoice showed subtotal 120.00, discount -50.00,
-- total 170.00 -- a "discount" that INCREASED the invoice total by 50
-- EGP, with no permission check and no dedicated discount audit-log
-- entry (that entry is also gated by `p_discount_amount > 0`). Any
-- staff member holding plain `booking.create` (not
-- `booking.discount.apply`) can inflate a customer's invoice this way.
--
-- Fix: reject any negative p_discount_amount outright, before the
-- existing `> 0` branch. A "discount" that raises the price is
-- nonsensical by definition -- this is a validation-gap fix, not a
-- new business rule (contrast with the pricing boundary-straddle
-- finding, D4, which genuinely is a business-policy question left for
-- the owner). Everything else in this function is byte-for-byte
-- identical to the prior version
-- (20260829270000_booking_discount_ceiling_fix.sql) -- only the new
-- guard clause is added, immediately before the pre-existing
-- `if p_discount_amount > 0` block.
...
begin
  ...
  -- D5 FIX: a discount can never be negative -- that would raise the
  -- price, not discount it, and would otherwise bypass every check in
  -- the block below (permission, 30% ceiling, exceeds-total).
