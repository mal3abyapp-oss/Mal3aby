-- Widen create_refund(): full-refund-cancels-membership rule.
-- Every other line preserved byte-for-byte from the live-captured version.
...
begin
  ...
  -- Club Membership integration: a FULL refund (this refund + all prior
  -- completed refunds on this payment == the full payment amount)
  -- against a payment allocated to a club_membership_subscriptions
  -- invoice cancels that membership period. A partial refund is still
  -- fully recorded via refunds/payment_allocations but does not
  -- auto-cancel (directive-mandated behavior).
