-- SAAS ACCEPTANCE REVIEW -- cross-module business logic audit finding
-- (2026-08-29): _create_booking_internal() has a 30%-without-override
-- discount ceiling, but no check anywhere that the discount doesn't
-- exceed the booking's own total price. A legitimately-permissioned
-- discount override (a real, intended capability -- staff holding
-- booking.discount.override, e.g. for goodwill/VIP cases) could set
-- p_discount_amount greater than v_total_price, producing a negative
-- invoices.total (subtotal - discount) and a negative invoice_items
-- line_total.
--
-- Contrast: create_shop_sale() already explicitly rejects
-- discount > subtotal; both Academy (create_enrollment_with_subscription)
-- and Club-Membership (sell_club_membership/renew_club_membership) use
-- greatest(price - discount, 0) to floor the result at zero. Booking
-- was the sole commerce-creating RPC missing an equivalent guard.
--
-- Fix: add an explicit "discount cannot exceed the booking total"
-- check, alongside (not replacing) the existing 30% ceiling -- both
-- checks apply; whichever is violated first is reported.
