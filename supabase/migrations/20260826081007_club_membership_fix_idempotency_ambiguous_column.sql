-- Fix: the idempotency-key lookup block in sell_club_membership and
-- renew_club_membership referenced the bare column name
-- `membership_subscription_id`, which is ambiguous with the RETURNS
-- TABLE column of the same name on both functions -- PL/pgSQL resolves
-- it as the OUT parameter, not the table column, causing a hard error
-- (42702) on every idempotent-replay call. Never exercised in earlier
-- testing because every prior test passed p_idempotency_key := null.
-- Fixed by aliasing the source table and qualifying the column.
...
begin
  ...
  insert into public.invoices (club_id, branch_id, invoice_number, customer_id, status, subtotal, discount, total, issued_at, created_by)
  values (v_current.club_id, v_current.branch_id, v_current.customer_id, v_invoice_number, 'issued', v_plan.price, coalesce(p_discount, 0), v_net_price, now(), auth.uid())
  returning id into v_invoice_id;
