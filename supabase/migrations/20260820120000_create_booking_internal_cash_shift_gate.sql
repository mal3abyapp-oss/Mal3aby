-- MASTER OPERATIONAL SIMPLIFICATION DIRECTIVE (2026-08-20), section 13-14:
-- "This applies to ALL cash collection paths ... Quick Booking ...
-- Do not fix only one screen." Real bug found by independent
-- verification agent: record_payment()'s custody+active-shift gate
-- (Phase D, 20260820074500) was never ported to
-- _create_booking_internal(), which is the function QuickBookingSheet's
-- inline "pay now" toggle actually calls (via create_booking()). A
-- custody-enabled staff member could collect cash through Quick
-- Booking's pay-now path with zero shift open, while the exact same
-- staff member is correctly blocked collecting cash through Booking
-- Detail/Billing (both call record_payment directly). Two different
-- payment-recording code paths had drifted -- a real duplication bug of
-- the kind this whole directive is trying to eliminate.
--
-- Fix: port the identical custody/shift lookup block from
-- record_payment() verbatim, and add cash_shift_id to this function's
-- payments insert (it was missing that column entirely, unlike
-- record_payment's insert).
...
declare
  ...
  v_receipt_id uuid;
  -- Phase D parity fix: same declarations as record_payment()'s gate.
...
begin
  ...
  -- Phase D parity fix: identical cash-custody + active-shift gate as
  -- record_payment(). Checked here (before any row is written) so a
  -- blocked cash payment never leaves a half-created booking behind --
  -- same "reject before writing" pattern already used for the
  -- government-receipt checks just below.
