...
-- 14/28-35: closes the most significant gap found in the Section-7
-- message-producer matrix audit -- NO Academy-specific WhatsApp
-- message existed anywhere in this codebase. create_enrollment_with_
-- subscription() correctly sends zero messages (enrollment/
-- subscription creation is silent by design, matching
-- _create_booking_internal's "silent create, message on the real paid
-- outcome" pattern -- confirmed no change needed there). The only
-- customer-facing consequence of an Academy payment was the generic
-- 'payment-received' template -- same wording as a field-booking
-- installment payment, with the receiving guardian's own name but
-- NEVER the player's name, violating Section 29 ("a guardian may have
-- multiple children -- must identify the specific player").
--
-- Fix: record_payment() now detects whether the invoice being paid
-- belongs to an Academy subscription (via subscriptions.invoice_id --
-- the exact same join the function already performs for its existing
-- v_pending_subscription_id / _activate_subscription_if_due_internal
-- step, so this reuses an already-established, already-correct
-- detection signal rather than inventing a new one) and, when it does,
-- queues 'academy-payment-received' instead of 'payment-received' --
-- never both (an if/else, matching the one-outcome-one-message
-- consolidation pattern _create_booking_internal already uses for its
-- own booking-created vs booking-confirmed-paid split). Passes
-- player_name (players.full_name via enrollments.player_id) and
-- group_name (groups.name) so the message can name the specific
-- player and academy/group, per Sections 29/30. Field-booking payments
-- (no matching subscription for this invoice) are completely
-- unaffected -- same 'payment-received' template, same variables, same
-- dedup key pattern, byte-for-byte unchanged behavior.
--
-- No other line in this function changed -- every existing guard
-- (idempotency, invoice lock, cancelled-booking rejection, cash-shift/
-- government-receipt validation, outstanding-balance math, the
-- booking-confirm race fix from migration 20260822070000) is preserved
-- exactly as-is.
...
declare
  ...
  -- Academy-message detection (Sections 14/28-35): resolved once,
  -- right where the function already looks up v_pending_subscription_id
  -- for the SAME invoice -- reuses that existing join rather than a
...
begin
  ...
  -- Academy detection (Sections 14/28-35): any subscription tied to
  -- this invoice, regardless of pending/active status -- a SECOND
  -- payment against an already-active subscription (e.g. a renewal
  -- installment) must still get the Academy-identified message, not
  -- just the very first payment that activated it. Resolved AFTER the
  -- activation step above so subscription_start_date/end_date reflect
  -- the real, current subscription row either way.
