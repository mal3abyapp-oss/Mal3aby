-- Owner-Level Review: WhatsApp message quality directive. The current
-- rendered message ("تم استلام حجزك في ملعب ملعب 1 - كرة قدم بتاريخ
-- 2026-08-17T07:00:00+00:00...") is unacceptable end-user copy: raw
-- ISO timestamps, no venue-timezone conversion, a duplicated "ملعب"
-- word, and no club/sport/booking-reference context. Root cause,
-- confirmed via a full audit of every queue_whatsapp_notification()
-- call site: the variables payloads never included club_name,
-- timezone, sport, customer_name, or a short human-readable booking
-- reference -- the connector's template renderer literally cannot
-- produce a good message with the data it's given today, no matter how
-- templates.ts itself is rewritten.
--
-- This migration is purely additive to the variables jsonb payloads
-- (new keys alongside the existing ones) -- it does not change
-- queue/retry/idempotency/dedup semantics, does not add a new schema
-- column (no booking_number column exists or is added; a short
-- reference is derived from the booking id's own first 8 hex chars,
-- consistent with how invoice_number is already the club's own
-- sequential reference), and every existing key stays present with the
-- same meaning so the connector's dedup_key strings and any other
-- consumer of these RPCs are unaffected. REPLACES the 4 business RPCs
-- that call queue_whatsapp_notification(); full bodies preserved from
-- their latest versions (20260817072521 for record_payment,
-- 20260817045250 for cancel_booking), only the variables payloads and
-- (for _create_booking_internal) two new local variables are added.

-- ============================================================
-- _create_booking_internal(): add club_name, timezone (already
-- resolved as v_timezone for local-date math -- just also passed
-- through), sport, customer_name, and a short booking reference.
-- ============================================================
...
-- ============================================================
-- cancel_booking(): add club_name, sport, customer_name, timezone,
-- booking_ref. Full body preserved from 20260817045250.
-- ============================================================
...
-- ============================================================
-- record_payment(): add club_name, customer_name, booking_ref (via
-- the invoice's linked booking, if any). Full body preserved from
-- 20260817072521.
-- ============================================================
...
-- ============================================================
-- create_refund(): add club_name, customer_name, invoice_number (via
-- the payment's own allocation, if any), booking_ref.
-- ============================================================
