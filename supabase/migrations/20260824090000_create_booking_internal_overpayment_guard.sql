-- FIX: _create_booking_internal had no overpayment guard, unlike its
-- sibling record_payment() (same base file, ~line 731: "raise exception
-- ... payment amount exceeds the invoice's outstanding balance").
--
-- Bug: the function inserted the raw p_payment_amount into
-- public.payments.amount, the payment.record audit log, the
-- payment.received notification event, and the customer-facing
-- WhatsApp/email 'amount_paid' field -- but only capped the actual
-- ledger allocation via least(p_payment_amount, v_total_price -
-- p_discount_amount) when inserting into payment_allocations. So a
-- staff member could pass p_payment_amount = 500 against a 200 EGP
-- invoice: payments.amount = 500, payment_allocations.amount = 200,
-- the customer is told "you paid 500 EGP", the audit trail permanently
-- misstates the amount collected, and any report/reconciliation that
-- sums payments.amount directly (cash-shift close, employee liability,
-- collections reports) overstates cash collected by the unallocated
-- surplus (300 EGP) versus what payment_allocations/invoice math shows.
--
-- Fix: mirror record_payment()'s existing behavior exactly -- reject
-- the payment outright with a clear exception if p_payment_amount
-- exceeds the invoice's payable amount (v_total_price -
-- p_discount_amount), before anything is inserted into public.payments.
-- This is a brand-new booking/invoice with no prior payments or
-- refunds, so the outstanding balance is simply v_total_price -
-- p_discount_amount -- no additional lookups are needed. The guard is
-- placed immediately before the existing 'payment.create' permission
-- check, i.e. still before any insert, so on rejection the booking
-- itself (already inserted earlier as 'pending_payment') is rolled
-- back along with everything else in this transaction, and no
-- payments/payment_allocations/audit rows or notifications are ever
-- created for the rejected amount. This is safe: it only closes a gap
-- that let payments.amount, the audit log, and customer notifications
-- disagree with the actual allocated/outstanding amount -- it does not
-- change behavior for any payment that was already within the correct
-- (non-overpaying) range, which is the only path production traffic
-- can have relied on being silently accepted.
...
begin
  ...
  -- FIX (20260824090000): reject overpayment up front, mirroring
  -- record_payment()'s existing guard. This is a brand-new invoice
  -- with no prior payments/refunds, so its outstanding balance is
  -- simply v_total_price - p_discount_amount.
