-- Government / Ministry Collection Compliance -- Onboarding question
-- (directive sections 3, 47).
--
-- Adds p_government_affiliated (default false, so any existing caller
-- signature keeps working with zero behavior change -- directive
-- section 47/48: "لا تمنع النادي التجاري العادي من التسجيل بسهولة" /
-- "إذا NO: كل النظام المالي الحالي يعمل كما هو"). When true, creates
-- the initial club-level government_collection_policies row atomically
-- in the same transaction as the club itself -- enabled but with
-- official_receipt_required left FALSE by default (the owner still
-- explicitly confirms the receipt requirement and payment methods in
-- Settings -> Finance & Compliance afterward; onboarding only captures
-- the affiliation fact itself, not the full compliance configuration,
-- consistent with directive section 7: "فعّل Compliance Setup" being a
-- distinct step from actually turning enforcement on).
...
begin
  ...
  -- Directive section 3/7: capture the affiliation fact at onboarding.
  -- official_receipt_required stays false here -- the owner confirms
  -- the actual enforcement + required payment methods afterward in
  -- Settings, so this never silently starts blocking payments before
  -- the owner has seen and configured the real policy.
...
revoke all on function public.complete_new_club_onboarding(text, text, text, text, text, text, text, text, boolean) from public;
revoke all on function public.complete_new_club_onboarding(text, text, text, text, text, text, text, text, boolean) from anon;
grant execute on function public.complete_new_club_onboarding(text, text, text, text, text, text, text, text, boolean) to authenticated;

-- Drop the old 8-arg signature -- fully replaced, not overloaded.
drop function if exists public.complete_new_club_onboarding(text, text, text, text, text, text, text, text);
