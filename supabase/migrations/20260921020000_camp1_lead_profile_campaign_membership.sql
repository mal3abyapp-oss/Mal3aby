-- CAMP-1 (owner brief, live QA 2026-09-19/20): get_lead_full_profile()
-- never surfaced which campaign(s) (if any) a lead belongs to -- a real
-- gap this migration closes alongside the sibling frontend fix (the
-- Manage-leads dialog on SalesCampaignsPage.tsx) and the sibling
-- sales-ai-offer-generator fix (deriving campaign_id server-side instead
-- of always persisting null). Adding this here, not duplicating it in
-- the Edge Function's own query, keeps a single source of truth for "is
-- this lead in a campaign" available to any future caller of this RPC,
...
begin
  ...
  select jsonb_build_object(
    ...
    -- CAMP-1 fix: which campaign(s) this lead belongs to, if any.
    'campaigns', coalesce((
      select jsonb_agg(jsonb_build_object('campaign_id', cam.id, 'name', cam.name, 'added_at', cl.added_at) order by cl.added_at desc)
      from public.sales_campaign_leads cl
      join public.sales_campaigns cam on cam.id = cl.campaign_id
...
-- Grants re-asserted explicitly rather than assumed preserved (this
-- session's own established discipline for every CREATE OR REPLACE,
-- even same-signature ones) -- checked via information_schema before
-- writing this: authenticated, postgres, AND service_role all currently
-- hold EXECUTE. No edge function calls this RPC (grep confirmed), so
-- the service_role grant is unused today, but it predates this fix and
-- this migration doesn't touch authorization scope -- preserved as-is
-- rather than silently narrowed.
revoke all on function public.get_lead_full_profile(uuid) from public, anon;
grant execute on function public.get_lead_full_profile(uuid) to authenticated, service_role;
