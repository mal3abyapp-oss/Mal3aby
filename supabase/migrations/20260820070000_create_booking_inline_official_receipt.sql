-- Phase B follow-up: the previous migration
-- (20260820064000_enforce_government_receipt_in_create_booking) added
-- the guard to _create_booking_internal() expecting a PRE-EXISTING
-- p_official_receipt_id -- but that's architecturally wrong for this
-- call path. record_payment_with_official_receipt() can require a
-- pre-existing receipt id because the invoice already exists by the
-- time payment is collected. _create_booking_internal() creates the
-- invoice AND the payment in the same transaction as the booking
-- itself -- there is no invoice, and therefore no correct
-- receipt_amount to validate against, until this function has already
-- computed v_total_price. A client can never correctly pre-create a
-- receipt row before calling create_booking().
--
-- Fix: accept the raw receipt fields (serial/date/book/series/image/
-- notes) directly, exactly like record_payment_with_official_receipt()
-- does, and INSERT the receipt row inline once v_total_price /
-- p_payment_amount are known -- mirroring that function's own insert
-- verbatim, not inventing new logic. p_official_receipt_id is removed;
-- nothing shipped with the old signature yet (this same session), so
-- there is no compatibility surface to preserve.
...
begin
  ...
  -- Phase C: reject a slot whose start time has already passed or
  -- started, even for staff.
  ...
  -- Phase B: mirror record_payment_with_official_receipt()'s own
  -- validation (serial/date required, date not in the future, image
  -- required if the policy demands it) BEFORE any booking/invoice/
  -- payment row is written, so a rejected payment never leaves a
  -- half-created booking behind. Resolution is field -> branch -> club,
  -- same precedence as record_payment().
...
-- Public wrapper: replace p_official_receipt_id with the raw receipt
-- fields, matching the internal function's real signature.
...
as $function$
  select public._create_booking_internal(
    ...
    null, p_receipt_serial, p_receipt_date, p_receipt_book, p_receipt_series, p_receipt_image_path, p_receipt_notes
  );
$function$;
