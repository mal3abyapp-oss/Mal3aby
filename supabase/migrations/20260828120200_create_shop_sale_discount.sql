-- Commerce Pro C3: extends create_shop_sale to accept and apply a real
-- discount at sale-creation time only (plan Section 2 -- discounts
-- never rewrite historical invoices; applied once, at creation, into
-- the existing invoices.discount column and the new
-- shop_sales.discount_amount/discount_reason columns).
--
-- Signature change: appended p_discount_amount numeric default 0,
-- p_discount_reason text default null -- matching the append-only
-- pattern this exact RPC family already established in C1
-- (create_shop_product/create_shop_category appended new params rather
-- than reordering existing ones). Every existing parameter, in its
-- existing order, is untouched, so every existing caller (none pass
-- discount args) continues to work identically -- discount_amount
-- defaults to 0, matching prior hardcoded behavior exactly.
--
-- Every other line of the function body is copied verbatim from the
-- current live definition (20260827003217_create_shop_sale_partial_payment.sql,
-- confirmed as the latest via direct migration read before writing
-- this file) -- auth checks, module-active gate, cash-shift gate,
-- idempotency handling, and audit-log calls are preserved exactly, not
...
-- Same grant-leak class as the earlier partial-payment migration:
-- appending parameters changes create_shop_sale's signature, which
-- Postgres treats as a new function identity and applies its own
-- default EXECUTE-to-PUBLIC grant on creation. Drop the prior 8-arg
-- overload explicitly and re-apply the exact same restrictive grants
-- (authenticated only) to the new 10-arg one -- matching
-- 20260827003243_fix_create_shop_sale_grant_leak_and_drop_orphaned_overload.sql's
-- own precedent, applied proactively in the same migration this time
-- rather than as a same-session follow-up fix.
drop function if exists public.create_shop_sale(uuid, uuid, uuid, jsonb, text, text, uuid, numeric);
