...
//
// CAMP-1 fix (owner brief, 2026-09-21): a campaign could be created but
// had NO way to ever add a single lead -- the backend RPC
// (sales_add_leads_to_campaign, inserting into sales_campaign_leads)
// worked correctly and always had, but no frontend component called it,
// so every campaign's target_count was permanently stuck at 0.
// CampaignLeadsDialog below closes that gap: "Manage leads" opens a
// picker (reusing search_sales_leads, the same RPC SalesLeadsPage.tsx
// already uses) with checkboxes, calling sales_add_leads_to_campaign on
// confirm. No backend change needed for this part --
// get_campaign_stats() already reads sales_campaign_leads correctly.
    ...
    <Card>
      ...
      <CardContent>
        ...
        {statsQuery.isLoading ? (
          ...
          <div className="grid grid-cols-3 gap-2 text-sm sm:grid-cols-7">
            ...
            {/* CAMP-1 fix: get_campaign_stats() has always computed
                `lost` and an i18n key already existed for it, but
                nothing rendered it -- an orphaned field. */}
            <span>{t('platform.sales.campaigns.stats.lost')}: {stats.lost}</span>
          </div>
        )}
      </CardContent>
    </Card>
