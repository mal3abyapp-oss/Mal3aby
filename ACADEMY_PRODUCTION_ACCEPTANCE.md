...
- AC7 = FIXED + PASS (closed under the CUSTOMER/PARENT EXPERIENCE directive, Section 2A). `get_my_portal_academy()` now uses the exact same `left join lateral (... order by created_at desc limit 1)` deterministic-selection pattern as `get_customer_academy_players()`, and inlines the freeze-sum effective-date math directly (the permission-gated `get_subscription_effective_end_date()` wrapper cannot be called from here -- a pure portal customer holds no `club_memberships` row, so `has_permission()` would silently return false/NULL for every portal customer). Live-verified all 7 required scenarios (single subscription; expired historical + current; renewed; frozen; previously-frozen-then-active; cancelled historical + newer; no active/current subscription) via a temporary test-only SQL wrapper mirroring the RPC's exact query (a genuine `auth.uid()`-based portal session wasn't available for this QA club -- no customer there has ever completed portal activation -- so the query logic itself was verified directly instead; the wrapper was dropped immediately after). New automated regression tests added to `academy-hardening.integration.test.ts`.
...
**UPDATE (closed under the CUSTOMER/PARENT EXPERIENCE directive, Section 2B)**: the `update_academy_membership`/RLS permission-key inconsistency described above was investigated further and CONFIRMED as a real semantic bug, not merely a benign inconsistency -- the permission catalog's own descriptions are unambiguous (`academy.program.manage` = "Create/update programs, seasons, age groups"; `academy.group.manage` = "Create/update groups and their weekly schedule"), and a full-codebase grep confirmed `academy.program.manage` was used as an RPC gate NOWHERE else in the entire system except this one (wrong) call site. Fixed: `update_academy_membership` now checks `academy.group.manage`, matching both the permission catalog's own semantic definition and the `groups` table's RLS policies it always disagreed with. This is a narrowing/correction, not a privilege broadening -- verified via a custom club role scoped to exactly `academy.group.manage` (no `academy.program.manage`), confirmed to have precisely that permission set, then cleaned up. Live-verified the positive path (an authorized `club_owner` caller) still succeeds correctly after the fix.
