...
Status: COMPLETE. Real fixture: 1 branch inserted via genuine RLS-authenticated write as the actual club owner (not a superuser bypass), producing real usage=1. `branch_limit` set to 0 via the real Platform Owner "تعديل الحدود" UI form. Found and fixed a genuine gap: the persistent limits display card had no visual indicator at all once an over-limit state was actually saved (only the pre-save warning inside the edit form existed) -- added a `StatusBadge tone="danger"` "فوق الحد"/"Over limit" badge plus an explanatory hint, matching the existing danger-badge visual language used elsewhere on this page. `tsc -b`/`eslint` clean. Live-verified: badge and hint render with correct danger styling (`bg-status-danger/10 text-status-danger border-status-danger/20`). Server-side enforcement independently confirmed via a real second-branch insert attempt while over limit: rejected with `branch_limit_reached`. Existing branch record confirmed preserved throughout (not deleted). Full restoration confirmed both via SQL and live UI: limit cleared back to unlimited via the real `set_commercial_entitlements` RPC (platform owner identity), fixture branch deleted via platform-owner-scoped RLS delete (branches has no club-staff delete policy by design -- only platform_owner has `ALL`/`*`; this is correct product design, not a gap, since branches are operational config restorable via UI toggle in the normal case, and this fixture was a genuinely disposable QA row, not financial/historical data), final state matches the exact original baseline (branch_count=0, all limits null, payments enabled, club active). One-time UI interaction friction encountered and worked around: the Browser pane's ref/coordinate-click and screenshot compositing became unreliable mid-session (stale coordinates, a stuck screenshot frame); recovered by closing and reopening the tab, and by using SQL ground-truth checks after every state-changing click to catch and immediately revert two unintended clicks (a stray club-suspend dialog opened-then-cancelled with no submit, and a stray payments-disable that was immediately restored via the real RPC) before proceeding -- documented as environment/tool friction per the directive's own §25 distinction, not a product defect.
