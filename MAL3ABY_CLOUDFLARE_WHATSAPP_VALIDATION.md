...
### 2.8 Queue recovery and duplicate-delivery protection confirmed real, with evidence of a prior live bug and fix

Read directly from the live deployed function source (not a migration filename assumption): `whatsapp_connector_claim_next_batch()` uses `FOR UPDATE OF ... SKIP LOCKED` for concurrency-safe claiming. A genuine crash-recovery gap — a connector crash mid-send left a queue row stuck in `processing` forever, invisible to every existing diagnostic — was found live during an earlier Safe Messaging test (`20260818100000_fix_stuck_processing_recovery.sql`) and fixed inside `whatsapp_connector_expire_stale()`, which `QueueConsumer.ts` already calls on every poll tick before claiming: any row stuck in `processing` for more than 10 minutes is reclaimed to `retrying` without double-counting its attempt, re-entering the normal capped-retry path. Combined with the partial unique index `notification_queue_dedup_active_idx` on `dedup_key` (scoped to active statuses, preventing a second row for the same logical event while one is in flight), this closes the queue-side duplicate-delivery risk at the row-creation level. The one residual, un-eliminable risk — a crash in the exact window between WhatsApp accepting a send and `reportSendResult()` committing — remains honestly documented rather than hidden, and is time-bounded by the same 10-minute recovery window.

### 2.9 Two real static-audit findings, fixed

- **Stale documentation-derived comment**: both the Dockerfile and `HealthServer.ts` described Cloudflare's Container readiness gate as a `pingEndpoint`/"/ready" path override. The actually-installed `@cloudflare/containers@0.0.13` API has no `pingEndpoint` property — `startAndWaitForPorts()` gates readiness on TCP port reachability only (confirmed by reading `node_modules/@cloudflare/containers/dist/index.d.ts` directly). Both comments corrected to describe the real mechanism; no behavior change, since the code never actually relied on the nonexistent property.
- **Non-constant-time token comparison**: `HealthServer.ts`'s `/status` endpoint and `cloudflare/whatsapp-worker/src/index.ts`'s `/manage/*` endpoint both used a plain `!==` string comparison for their bearer tokens. `/manage/*` is this architecture's one genuinely public-internet-facing route, making this a real (if narrow) threat surface, not merely theoretical. Fixed: the Container's internal `/status` check now uses `node:crypto`'s `timingSafeEqual`; the Worker's public `/manage/*` check now uses the Workers runtime's native `crypto.subtle.timingSafeEqual` (Cloudflare's own documented pattern, including never short-circuiting on a length mismatch, which would itself leak the secret's length via timing). Both typechecked clean and were redeployed live; the Management API auth gate was re-verified live afterward (401/401/404 for no-token/wrong-token/unknown-route).
