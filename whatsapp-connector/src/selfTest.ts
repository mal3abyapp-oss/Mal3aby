...
async function main() {
  ...
  let qrSeen = false
  let sawWaitingForScan = false

  const provider = new BaileysMessagingProvider(TEST_CLUB_ID, {
    onStateChange: (state) => {
      console.log('[selfTest] state ->', state)
      if (state === 'waiting_for_scan') sawWaitingForScan = true
    },
  })
  ...
  if (!qrSeen) {
    ...
    process.exitCode = 1
  } else if (!sawWaitingForScan) {
    console.error('[selfTest] FAILED — QR was received but state never transitioned to waiting_for_scan.')
    process.exitCode = 1
  } else {
    console.log('[selfTest] PASSED — real Baileys handshake reaches waiting_for_scan with a real QR payload.')
